...
export const runSeedRoles = async (opts: SeedOptions = {}): Promise<void> => {
  const { manageConnection = true, silent = false } = opts;
  if (manageConnection) {
    if (!silent) console.log('Connecting to database...');
    await connectDB();
  }

  if (!silent) console.log('--- STARTING ROLE SEEDING (UPSERT) ---');
  ...
  for (const roleData of SYSTEM_ROLES) {
    ...
    if (!silent) {
      console.log(
        `${created ? 'CREATED' : 'EXISTS/UPDATED'}: ${role.name} with ID: ${role.id}`,
      );
    }
  }

  if (!silent) console.log('--- ROLE SEEDING COMPLETE ---');
