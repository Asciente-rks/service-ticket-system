...
import { getOwnProfile, updateOwnProfile } from '../controllers/profile.controller';
...
import { createUserSchema,updateUserSchema,userIdParamsSchema,updateProfileSchema} from '../../../utils/user.validation';
...
// Self-service profile. MUST be declared before the '/:id' routes below so the
// literal 'me' segment isn't captured by the UUID-validated param routes.
userRouter.get('/me', authenticateToken, getOwnProfile);
userRouter.patch('/me', authenticateToken, validate(updateProfileSchema), updateOwnProfile);
